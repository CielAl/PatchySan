function [net,stats] = cnn_train_dag(net, graphDB, getBatch, varargin)
...
if isempty(opts.train), opts.train = find(graphDB.data.set==1) ; end
if isempty(opts.val), opts.val = find(graphDB.data.set==2) ; end
...
if strcmp(mode,'train')
  ...
  state.cache=num2cell(zeros(1, numel(net.params))); %mark
end
...
if numGpus >= 1
  ...
  if strcmp(mode,'train')
    state.momentum = cellfun(@gpuArray,state.momentum,'UniformOutput',false) ;
     state.momentum = cellfun(@gpuArray,state.cache,'UniformOutput',false) ;
  end
end
if numGpus > 1
...
if ~isempty(pre)
    ...
    inputs = state.getBatch(state.graphDB, batch,opts) ;
end
    if opts.prefetch
      ...
      state.getBatch(state.graphDB, nextBatch) ;
    end
  ...
  switch net.params(p).trainMethod
    ...
    case 'average' % mainly for batch normalization
      ...
      net.params(p).value = ...
          ...
          (thisLR/double(batchSize)/net.params(p).fanout) * net.params(p).der ;
    ...
    case 'gradient'
      ...
      state.momentum{p} = opts.momentum * state.momentum{p} ...
        ...
        - (1 / double(batchSize)) * net.params(p).der ;
